...
-- Desync antigo removido — substituído pelo Mobile Desync abaixo
...
local Theme = {
    ...
    Surface         = Color3.fromRGB(35, 8, 8),
    SurfaceHighlight= Color3.fromRGB(55, 14, 14),
    Accent1         = Color3.fromRGB(220, 30, 30),
...
local function ApplyViewportUIScale(targetFrame, designWidth, designHeight, minScale, maxScale)
    ...
    if SharedState.RefreshMobileScale then
        ...
        sc.Scale = math.clamp(tonumber(Config.MobileGuiScale) or 1.0, 0.5, 2)
    end
end

SharedState.RefreshMobileScale = function()
    local s = math.clamp(tonumber(Config.MobileGuiScale) or 1.0, 0.5, 2)
...
local function AddMobileMinimize(frame, labelText)
    ...
    restoreBtn.Size = UDim2.new(0, 140, 0, 44)
...
local function ShowNotification(title, text)
    ...
    f.Position = UDim2.new(0.5, -170, 0, 80)
    f.BackgroundColor3 = Color3.fromRGB(10, 4, 4)
...
task.spawn(function()
    ...
    screenGui.Name = "WSHub_UI"; screenGui.ResetOnSpawn = false; screenGui.Parent = PlayerGui
    ...
    frame.BackgroundColor3 = Color3.fromRGB(10, 4, 4)
    ...
    headerFix.BackgroundColor3 = Color3.fromRGB(30, 8, 8)
    ...
    if IS_MOBILE then
        ...
        menuToggleBtn.Size = UDim2.new(0, 80, 0, 32)
    ...
    statusRow.BackgroundColor3 = Color3.fromRGB(30, 8, 8)
    ...
    progressBg.BackgroundColor3 = Color3.fromRGB(30, 8, 8)
    progressBg.BorderSizePixel = 0
    local progressBarFill = Instance.new("Frame", progressBg)
    progressBarFill.Size = UDim2.new(0, 0, 1, 0)
    progressBarFill.BackgroundColor3 = Color3.fromRGB(220, 30, 30)
    ...
    stealModeFrame.BackgroundColor3 = Color3.fromRGB(15, 4, 4)
    ...
    smStroke.Color = Color3.fromRGB(80, 20, 20); smStroke.Thickness = 1.2
    local smHeader = Instance.new("Frame", stealModeFrame)
    smHeader.Size = UDim2.new(1, 0, 0, 52); smHeader.BackgroundColor3 = Color3.fromRGB(25, 6, 6); smHeader.BorderSizePixel = 0
    ...
    smTitle.Text = "WS HUB"; smTitle.Font = Enum.Font.GothamBlack; smTitle.TextSize = 14
    ...
    local function ShowPriorityAlert(brainrotName, genText, mutation, ownerUsername)
        ...
        local color = mutationColors[normalizedMutation] or Color3.fromRGB(220, 30, 30)
...
frame.Size = UDim2.new(0, 340*mobileScale, 0, 490*mobileScale)
    ...
    local stroke = Instance.new("UIStroke", frame)
    stroke.Color = Color3.fromRGB(80, 20, 20); stroke.Thickness = 1.5; stroke.Transparency = 0.2

    local header = Instance.new("Frame", frame)
    header.Size = UDim2.new(1, 0, 0, 48)
    header.BackgroundColor3 = Color3.fromRGB(25, 6, 6)
    header.BorderSizePixel = 0
    ...
    hdrAccent.BackgroundColor3 = Color3.fromRGB(200, 30, 30); hdrAccent.BorderSizePixel = 0
    ...
    refreshBtn.BackgroundColor3 = Color3.fromRGB(160, 30, 30)
    ...
    refreshStroke.Color = Color3.fromRGB(200, 50, 50)
    ...
    proxCont.BackgroundColor3 = Color3.fromRGB(20, 5, 5)
    ...
    proxContStroke.Color = Color3.fromRGB(80, 20, 20)
    ...
    proxBtn.BackgroundColor3 = ProximityAPActive and Color3.fromRGB(220, 30, 30) or Color3.fromRGB(40, 18, 18)
    proxBtn.Text = "Prox"
    ...
    spamBaseBtn.BackgroundColor3 = Color3.fromRGB(40, 18, 18)
    ...
    local function updateCtapPanelBtn()
        ctapPanelBtn.BackgroundColor3 = Config.ClickToAP and Color3.fromRGB(220, 30, 30) or Color3.fromRGB(40, 18, 18)
    ...
    local function setActiveTab(name)
        ...
        if name == "players" then
            tabPlayers.BackgroundColor3 = Color3.fromRGB(220, 30, 30)
...
highlight.Adornee = newAdornee
    ...
    local function createPlayerRow(plr)
        ...
        row.Name = plr.Name
        row.LayoutOrder = 0
row.Size = UDim2.new(1, -4, 0, 48)        row.BackgroundColor3 = Color3.fromRGB(18, 5, 5)
        ...
        row.MouseLeave:Connect(function()
            row.BackgroundTransparency = 0.0
            row.BackgroundColor3 = Color3.fromRGB(18, 5, 5)
            rowStroke.Transparency = 0.5
        end)
        ...
        headshotStroke.Color = Color3.fromRGB(180, 40, 40)
...
local function runAutoSnipeV2()
    ...
    local ok, err = pcall(function()
        ...
        if hrp.Position.Y > 25 and targetPosition.Y > 25 then
            ...
            -- Equipa o tapete para o servidor aceitar o movimento
            local carpetName = Config.TpSettings.Tool or "Flying Carpet"
            local carpet = LocalPlayer.Backpack:FindFirstChild(carpetName) or char:FindFirstChild(carpetName)
            if carpet and hum and hum.Parent then hum:EquipTool(carpet) end

            if not char.Parent or not hrp.Parent then finishTeleport(); return end

            -- Sobe via velocidade (SEM ChangeState — causa morte)
            local jumpStart = tick()
            while hrp.Position.Y < 40 and (tick() - jumpStart) < 2 do
                hrp.AssemblyLinearVelocity = Vector3.new(hrp.AssemblyLinearVelocity.X, 200, hrp.AssemblyLinearVelocity.Z)
                RunService.Heartbeat:Wait()
            end
            task.wait(0.05)
            ...
            -- TP direto para finalPos — sem passar pelo X do tapete
            hrp.AssemblyLinearVelocity = Vector3.zero
            hrp.AssemblyAngularVelocity = Vector3.zero
            hrp.CFrame = CFrame.new(finalPos) * facingRot
            hrp.AssemblyLinearVelocity = Vector3.zero
            ...
            -- BodyPosition segura contra lagback por 300ms
            local holdBP = Instance.new("BodyPosition")
            holdBP.Name     = "TempHoldBP"
            holdBP.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
            holdBP.P        = 50000
            holdBP.D        = 5000
            holdBP.Position = finalPos
            holdBP.Parent   = hrp
            task.delay(0.3, function()
                if holdBP and holdBP.Parent then holdBP:Destroy() end
            end)
        end
...
if IS_MOBILE then
    -- ══════════════════════════════════════════════════════════════════
    -- WS HUB — PAINEL LATERAL SLIDE (MOBILE)
    -- Aba visivel na lateral direita, desliza para abrir
    -- ══════════════════════════════════════════════════════════════════
    ...
    -- Painel principal (começa fora da tela pela direita)
    local panel = Instance.new("Frame")
    panel.Name = "WSHubSidePanel"
    panel.Size = UDim2.new(0, PANEL_W, 0, PANEL_H)
    panel.Position = UDim2.new(1, 0, 0.5, -PANEL_H/2)
    panel.BackgroundColor3 = Color3.fromRGB(12, 5, 5)
    ...
    panelStroke.Color = Color3.fromRGB(180, 25, 25)
    panelStroke.Thickness = 2
    panelStroke.Transparency = 0.2

    -- Aba lateral visivel sempre
    local tab = Instance.new("TextButton")
    tab.Name = "WSHubTab"
    tab.Size = UDim2.new(0, TAB_W, 0, TAB_H)
    tab.Position = UDim2.new(0, -TAB_W, 0.5, -TAB_H/2)
    tab.BackgroundColor3 = Color3.fromRGB(180, 20, 20)
    ...
    hFix.BackgroundColor3 = Color3.fromRGB(30, 8, 8)
    hFix.BorderSizePixel = 0

    local hLogo = Instance.new("Frame", header)
    hLogo.Size = UDim2.new(0, 30, 0, 30)
    hLogo.Position = UDim2.new(0, 12, 0.5, 0)
    hLogo.AnchorPoint = Vector2.new(0, 0.5)
    hLogo.BackgroundColor3 = Color3.fromRGB(180, 20, 20)
    hLogo.BorderSizePixel = 0
    Instance.new("UICorner", hLogo).CornerRadius = UDim.new(0, 8)
    local hLogoLbl = Instance.new("TextLabel", hLogo)
    hLogoLbl.Size = UDim2.new(1, 0, 1, 0)
    hLogoLbl.BackgroundTransparency = 1
    hLogoLbl.Text = "W"
    hLogoLbl.Font = Enum.Font.GothamBlack
    hLogoLbl.TextSize = 16
    hLogoLbl.TextColor3 = Color3.fromRGB(255, 255, 255)

    local hTitle = Instance.new("TextLabel", header)
    hTitle.Size = UDim2.new(0, 110, 0, 18)
    hTitle.Position = UDim2.new(0, 50, 0, 8)
    hTitle.BackgroundTransparency = 1
    hTitle.Text = "WS HUB"
    hTitle.Font = Enum.Font.GothamBlack
    hTitle.TextSize = 15
    hTitle.TextColor3 = Color3.fromRGB(230, 230, 240)
    hTitle.TextXAlignment = Enum.TextXAlignment.Left

    local hSub = Instance.new("TextLabel", header)
    hSub.Size = UDim2.new(0, 110, 0, 13)
    hSub.Position = UDim2.new(0, 50, 0, 28)
    hSub.BackgroundTransparency = 1
    hSub.Text = "Mobile Panel"
    hSub.Font = Enum.Font.GothamMedium
    hSub.TextSize = 10
    hSub.TextColor3 = Color3.fromRGB(120, 50, 50)
    hSub.TextXAlignment = Enum.TextXAlignment.Left

    -- Scroll area
    local scrollArea = Instance.new("ScrollingFrame", panel)
    scrollArea.Size = UDim2.new(1, 0, 1, -58)
    scrollArea.Position = UDim2.new(0, 0, 0, 56)
    ...
    listLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        scrollArea.CanvasSize = UDim2.new(0, 0, 0, listLayout.AbsoluteContentSize.Y + 16)
    end)

    -- Cria linha de botao no painel
    -- Painel usa FlexibleLayout invertido: botoes mais importantes ficam embaixo
    -- Invertemos a ordem do UIListLayout para que order=1 fique no final (mais acessível)
    listLayout.VerticalAlignment = Enum.VerticalAlignment.Bottom
    listPad.PaddingTop = UDim.new(0, 8)
    listPad.PaddingBottom = UDim.new(0, 10)

    local function createSideBtn(label, accentColor, order, callback)
        local row = Instance.new("Frame", scrollArea)
        row.Name = "Row_" .. label
        row.Size = UDim2.new(1, -14, 0, 54)
        row.BackgroundColor3 = Color3.fromRGB(28, 8, 8)
        row.BorderSizePixel = 0
        ...
        nameLbl.TextSize = 15
        nameLbl.TextColor3 = Color3.fromRGB(225, 225, 240)
        nameLbl.TextXAlignment = Enum.TextXAlignment.Left

        -- Status (linha 2)
        local stateLbl = Instance.new("TextLabel", row)
        stateLbl.Size = UDim2.new(1, -28, 0, 14)
        stateLbl.Position = UDim2.new(0, 22, 0, 32)
        stateLbl.BackgroundTransparency = 1
        stateLbl.Text = "—"
        stateLbl.Font = Enum.Font.GothamMedium
        stateLbl.TextSize = 11
        stateLbl.TextColor3 = Color3.fromRGB(90, 55, 55)
        ...
        btn.MouseButton1Click:Connect(function()
            row.BackgroundColor3 = Color3.fromRGB(55, 14, 14)
            task.delay(0.13, function() row.BackgroundColor3 = Color3.fromRGB(28, 8, 8) end)
            callback(btn, stateLbl, nameLbl)
        end)
        return {btn=btn, stateLbl=stateLbl, nameLbl=nameLbl, row=row, stroke=rowStroke}
    end

    -- ── BOTÕES (order alto = topo, order baixo = base/mais acessível) ─

    -- Menos usados: topo
    createSideBtn("Settings", Color3.fromRGB(160, 120, 40), 8, function(btn, lbl)
        if settingsGui then
    ...
    createSideBtn("Toggle UI", Color3.fromRGB(120, 120, 180), 7, function(btn, lbl)
        ...
        lbl.Text = "Alternado"
        task.delay(1.2, function() lbl.Text = "—" end)
        ShowNotification("WS HUB", "UI alternada")
    end)

    createSideBtn("Kick", Color3.fromRGB(180, 40, 40), 6, function(btn, lbl)
        lbl.Text = "Saindo..."
    ...
    -- Divisor visual
    local divider = Instance.new("Frame", scrollArea)
    divider.Size = UDim2.new(1, -14, 0, 1)
    divider.BackgroundColor3 = Color3.fromRGB(60, 18, 18)
    divider.BorderSizePixel = 0
    divider.LayoutOrder = 5

    -- Mais usados: base
    local dsRef = createSideBtn("Desync", Color3.fromRGB(255, 80, 80), 4, function(btn, lbl, name)
        ...
        if dsBtn then
            ...
            task.delay(0.15, function()
                local on = dsBtn.TextColor3 == Color3.fromRGB(0, 255, 0)
                lbl.Text = on and "ATIVO" or "Inativo"
                lbl.TextColor3 = on and Color3.fromRGB(0,220,70) or Color3.fromRGB(90,55,55)
            end)
        else
            ShowNotification("DESYNC", "Nao disponivel")
        end
    end)

    local spActive = false
    local spRef = createSideBtn("Speed", Theme.Success, 3, function(btn, lbl)
        spActive = not spActive
        carpetSpeedEnabled = spActive
        setCarpetSpeed(spActive)
        lbl.Text = spActive and "ATIVO" or "Inativo"
        lbl.TextColor3 = spActive and Theme.Success or Color3.fromRGB(90,55,55)
        ShowNotification("WS HUB", spActive and "Speed ON" or "Speed OFF")
    end)

    createSideBtn("Invisible", Color3.fromRGB(255, 60, 60), 2, function(btn, lbl)
        if _G.toggleInvisibleSteal then
            _G.toggleInvisibleSteal()
            task.delay(0.12, function()
                local on = _G.invisibleStealEnabled
                lbl.Text = on and "ATIVO" or "Inativo"
                lbl.TextColor3 = on and Color3.fromRGB(255,60,60) or Color3.fromRGB(90,55,55)
                ShowNotification("WS HUB", on and "Invis ON" or "Invis OFF")
            end)
        end
    end)

    -- O mais importante: bem no fundo, polegar alcança fácil
    createSideBtn("Teleport", Theme.Accent1, 1, function(btn, lbl)
        ...
        task.delay(1.5, function() lbl.Text = "—"; lbl.TextColor3 = Color3.fromRGB(90,55,55) end)
    end)

    -- Reset posições (fora do scroll, no rodapé do painel)
    local footer = Instance.new("Frame", panel)
    footer.Size = UDim2.new(1, 0, 0, 38)
    footer.Position = UDim2.new(0, 0, 1, -38)
    footer.BackgroundColor3 = Color3.fromRGB(20, 6, 6)
    footer.BorderSizePixel = 0
    local footerFix = Instance.new("Frame", footer)
    footerFix.Size = UDim2.new(1, 0, 0.5, 0)
    footerFix.BackgroundColor3 = Color3.fromRGB(20, 6, 6)
    footerFix.BorderSizePixel = 0
    Instance.new("UICorner", footer).CornerRadius = UDim.new(0, 18)
    ...
    rstBtn.TextSize = 12
    rstBtn.TextColor3 = Color3.fromRGB(100, 50, 50)
    ...
    rstBtn.MouseButton1Click:Connect(function()
        ...
        rstBtn.TextColor3 = Theme.Success
        task.delay(1.5, function() rstBtn.Text = "Reset posicoes"; rstBtn.TextColor3 = Color3.fromRGB(100,50,50) end)
        ShowNotification("RESET", "Posicoes e escala resetadas")
    end)

    -- Ajustar scroll para não cobrir o footer
    scrollArea.Size = UDim2.new(1, 0, 1, -96)
    ...
    local function openPanel()
        ...
        TweenService:Create(panel, tweenInfo, {
            Position = UDim2.new(1, -PANEL_W, 0.5, -PANEL_H/2)
        }):Play()
    end
    ...
    local function closePanel()
        ...
        tab.Text = "WS"
        TweenService:Create(panel, tweenInfo, {
            Position = UDim2.new(1, 0, 0.5, -PANEL_H/2)
        }):Play()
    end
    ...
    -- Ocultar botao desync antigo
    task.defer(function()
        local sg = PlayerGui:FindFirstChild("WSHubMobileDesyncGui")
        if sg then
            local oldBtn = sg:FindFirstChild("WSHubMobileDesyncBtn")
            if oldBtn then oldBtn.Visible = false end
        end
    end)
end
...
local rHomeKick = CreateRow("Home Kick (volta pro Roblox)")
local bHomeKick = Instance.new("TextButton", rHomeKick)
bHomeKick.Size=UDim2.new(0,60,0,24); bHomeKick.Position=UDim2.new(1,-70,0.5,-12)
bHomeKick.BackgroundColor3=Theme.SurfaceHighlight
bHomeKick.Text=Config.HomeKickKey ~= "" and Config.HomeKickKey or "NONE"
bHomeKick.Font=Enum.Font.GothamBold; bHomeKick.TextColor3=Theme.TextPrimary; bHomeKick.TextSize=12
Instance.new("UICorner",bHomeKick).CornerRadius=UDim.new(0,4)
bHomeKick.MouseButton1Click:Connect(function()
    bHomeKick.Text="..."; bHomeKick.TextColor3=Theme.Accent1
    local con; con=UserInputService.InputBegan:Connect(function(inp)
        if inp.UserInputType==Enum.UserInputType.Keyboard then
            Config.HomeKickKey=inp.KeyCode.Name; bHomeKick.Text=inp.KeyCode.Name
            bHomeKick.TextColor3=Theme.TextPrimary; SaveConfig(); con:Disconnect()
            ShowNotification("HOME KICK KEYBIND", inp.KeyCode.Name)
        end
    end)
end)
...
if IS_MOBILE then
    ...
    scaleUI.readout.Text = string.format("%.1f", Config.MobileGuiScale or 1.0)
    ...
    task.defer(function() scaleUI.updateScaleSlider(Config.MobileGuiScale or 1.0) end)
...
bReset.MouseButton1Click:Connect(function()
    ...
    if PlayerGui:FindFirstChild("WSHub_UI") then
...
-- ── REMOTE SELL section in settings ────────────────────────────
CreateSectionHeader("REMOTE SELL")
local rRemoteSell = CreateRow("Remote Sell Panel")
CreateToggleSwitch(rRemoteSell, Config.RemoteSellEnabled, function(ns, set)
    set(ns)
    Config.RemoteSellEnabled = ns
    SaveConfig()
    if _G.toggleRemoteSell then
        _G.toggleRemoteSell(ns)
    end
end)
...
task.spawn(function()
    ...
    local HTheme = {
        ...
        Accent1 = Color3.fromRGB(220, 30, 30),
        Accent2 = Color3.fromRGB(180, 15, 15),
    ...
    serverPosFrame.BackgroundColor3 = Color3.fromRGB(10, 4, 4)
        ...
        if state then
            ...
            if isInTargetBase and floorOk then
                ...
                if isFloor2to3 then
                    ...
                    platform.Color = Color3.fromRGB(220, 30, 30)
...
do
    ...
    abToggleFrame.Size            = UDim2.new(0, 160, 0, 42)
    ...
    abLabel.Position = UDim2.new(0, 10, 0, 0)
    ...
    abDot.Size = UDim2.new(0, 15, 0, 15)
...
do
    ...
    qpFrame.BackgroundColor3 = Color3.fromRGB(12, 4, 4)
    ...
    qpStroke.Color = Color3.fromRGB(120, 20, 20); qpStroke.Thickness = 1.5

    -- Header
    local qpHeader = Instance.new("Frame", qpFrame)
    qpHeader.Size = UDim2.new(1, 0, 0, 48)
    qpHeader.BackgroundColor3 = Color3.fromRGB(22, 6, 6)
...
do
    ...
    local function buildRemoteSellGUI()
        ...
        accent_rs.Size = UDim2.new(1,0,0,2); accent_rs.Position = UDim2.new(0,0,0,44)
        ...
        minBtn_rs.Size = UDim2.new(0,26,0,26); minBtn_rs.Position = UDim2.new(1,-32,0.5,-13)
        ...
        titleLabel_rs.Size = UDim2.new(1,-60,1,0); titleLabel_rs.Position = UDim2.new(0,12,0,0)
        titleLabel_rs.BackgroundTransparency = 1; titleLabel_rs.Text = "REMOTE SELL"
        titleLabel_rs.TextColor3 = Theme.Accent1; titleLabel_rs.Font = Enum.Font.GothamBlack
        ...
        sellAllBtn_rs.Size = UDim2.new(1,-18,0,30); sellAllBtn_rs.Position = UDim2.new(0,9,0,52)
        ...
        scanBtn_rs.Size = UDim2.new(1,-18,0,26); scanBtn_rs.Position = UDim2.new(0,9,0,88)
        ...
        statusLabel_rs.Size = UDim2.new(1,-18,0,16); statusLabel_rs.Position = UDim2.new(0,9,0,120)
        ...
        scrollFrame_rs.Size = UDim2.new(1,-18,1,-148); scrollFrame_rs.Position = UDim2.new(0,9,0,140)
        ...
        scrollFrame_rs.CanvasSize = UDim2.new(0,0,0,0); scrollFrame_rs.ZIndex = 3
        ...
        local function addRow_rs(podiumNum, sellPrompt, animalInfo)
            ...
            nameLabel_rs.Size = UDim2.new(1,-100,0,18); nameLabel_rs.Position = UDim2.new(0,40,0,5)
            ...
            subLabel_rs.Size = UDim2.new(1,-100,0,13); subLabel_rs.Position = UDim2.new(0,40,0,25)
            ...
            sellBtn_rs.Size = UDim2.new(0,52,0,28); sellBtn_rs.Position = UDim2.new(1,-58,0.5,-14)
...
task.spawn(function()
    ...
    sg.Name         = "WSHubMobileDesyncGui"
    sg.ResetOnSpawn = false
    sg.IgnoreGuiInset = true
    sg.DisplayOrder = 999

    local btn = Instance.new("TextButton", sg)
    btn.Name              = "WSHubMobileDesyncBtn"
    btn.Size              = UDim2.new(0, 160, 0, 58)
    btn.Position          = UDim2.new(0.94, 0, 0.10, 0)
    ...
    local function respawnChar(plr)
        ...
        local dummy = Instance.new("Model"); dummy.Parent = workspace
        plr.Character = dummy; task.wait(); plr.Character = char
        dummy:Destroy()
    end

    local function makeServerPos()
        if serverPos then serverPos:Destroy() end
        local p = Instance.new("Part")
        p.Name = "NHServerPosition"; p.Size = Vector3.new(4,4,4)
        p.Shape = Enum.PartType.Ball; p.Material = Enum.Material.Neon
        p.Color = Color3.fromRGB(0,255,0); p.Transparency = 0.5
        p.Anchored = true; p.CanCollide = false; p.Parent = workspace
        serverPos = p
    end

    local function syncPos()
        if not isEnabled or not serverPos then return end
        local char = lp.Character
        local root = char and char:FindFirstChild("HumanoidRootPart")
        if root then serverPos.CFrame = root.CFrame end
    end

    local movementKeys = {Enum.KeyCode.W, Enum.KeyCode.A, Enum.KeyCode.S, Enum.KeyCode.D}
    local isMoving = false
    UserInputSvc.InputBegan:Connect(function(i, gp)
        if gp then return end
        if table.find(movementKeys, i.KeyCode) then isMoving = true end
    end)
    UserInputSvc.InputEnded:Connect(function(i)
        if table.find(movementKeys, i.KeyCode) then
            isMoving = false; syncPos()
        end
    end)
    ...
    local function watchChar(char)
        ...
        local conn; conn = root:GetPropertyChangedSignal("CFrame"):Connect(function()
            if not isEnabled then conn:Disconnect(); return end
            if not isMoving then syncPos() end
        end)
    end
    ...
    local function disable()
        ...
        if serverPos then serverPos:Destroy(); serverPos = nil end
    end

    lp.CharacterAdded:Connect(function(char)
        if isEnabled then watchChar(char) end
    end)
    ...
    -- Nao ativa automaticamente — usuario precisa clicar no botao
end)
-- ══════════════════════════════════════════════════════════════════
-- FIM MOBILE DESYNC
-- ══════════════════════════════════════════════════════════════════
