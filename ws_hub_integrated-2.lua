        ...
        _sentryConn = RunService.Heartbeat:Connect(function()
            ...
            if _sentryTarget and _sentryTarget.Parent == Workspace then
                ...
                if _sentryTarget and _sentryTarget ~= prev then
                    ShowNotification("ANTI SENTRY", "Torreta detectada!")
                end
            end
        end)
    end
...
local function setRaknetDesync(enabled)
    ...
    ShowNotification("DESYNC RAKNET", status)
end
...
-- ── QUICK ACTIONS PANEL ─────────────────────────────────────────
do -- QuickActions Panel (escopo isolado)

    -- ── Cores próprias do painel (independente do tema) ───────────
    local QA = {
        Bg          = Color3.fromRGB(10, 10, 10),       -- fundo principal
        Header      = Color3.fromRGB(18, 18, 18),       -- barra de título
        Border      = Color3.fromRGB(200, 40, 40),      -- borda vermelha hellcat
        BtnBase     = Color3.fromRGB(22, 22, 22),       -- botão normal
        BtnHover    = Color3.fromRGB(38, 38, 38),       -- botão hover
        BtnAccent   = Color3.fromRGB(200, 40, 40),      -- botão destaque (vermelho)
        BtnAccentHv = Color3.fromRGB(230, 60, 60),      -- destaque hover
        ToggleOff   = Color3.fromRGB(30, 30, 30),       -- toggle desligado
        ToggleOn    = Color3.fromRGB(200, 40, 40),      -- toggle ligado
        TxtMain     = Color3.fromRGB(230, 230, 230),    -- texto principal
        TxtSub      = Color3.fromRGB(130, 130, 130),    -- texto secundário
        Dot         = Color3.fromRGB(60, 200, 90),      -- indicador ativo
        DotOff      = Color3.fromRGB(100, 100, 100),    -- indicador inativo
        Divider     = Color3.fromRGB(35, 35, 35),       -- linha separadora
    }
    ...
    -- ── Tamanho base ──────────────────────────────────────────────
    local W, H_HEADER = 190, 28
    local FULL_HEIGHT  = 292   -- altura expandida
    local MIN_HEIGHT   = H_HEADER
    ...
    local function label(parent, text, size, color, font, xalign)
        ...
        l.TextColor3 = color or QA.TxtMain
    ...
    local existingQA = PlayerGui:FindFirstChild("WS_QuickActions")
    if existingQA then existingQA:Destroy() end

    local sg = Instance.new("ScreenGui")
    sg.Name             = "WS_QuickActions"
    ...
    root.Name              = "QARoot"
    root.Size              = UDim2.new(0, W, 0, FULL_HEIGHT)
    root.Position          = UDim2.new(
        Config and Config.Positions and Config.Positions.QuickActions and Config.Positions.QuickActions.X or 0.78,
        0,
        Config and Config.Positions and Config.Positions.QuickActions and Config.Positions.QuickActions.Y or 0.22,
        0
    )
    root.BackgroundColor3  = QA.Bg
    root.BorderSizePixel   = 0
    root.ClipsDescendants  = true
    corner(root, 8)
    stroke(root, QA.Border, 1, 0)

    -- Linha de acento no topo (barra vermelha fina)
    local topBar = Instance.new("Frame", root)
    topBar.Size             = UDim2.new(1, 0, 0, 2)
    topBar.Position         = UDim2.new(0, 0, 0, 0)
    topBar.BackgroundColor3 = QA.Border
    ...
    header.BackgroundColor3 = QA.Header
    ...
    dot.BackgroundColor3   = QA.Dot
    dot.BorderSizePixel    = 0
    corner(dot, 99)

    -- Título
    local titleLbl = label(header, "⚡ QUICK ACTIONS", 10, QA.TxtMain, Enum.Font.GothamBlack)
    ...
    minBtn.BackgroundColor3  = QA.BtnBase
    minBtn.BorderSizePixel   = 0
    minBtn.Font              = Enum.Font.GothamBold
    minBtn.TextSize          = 11
    minBtn.TextColor3        = QA.TxtSub
    minBtn.Text              = "─"
    corner(minBtn, 4)
    ...
    body.Name             = "Body"
    body.Size             = UDim2.new(1, 0, 1, -(H_HEADER + 4))
    body.Position         = UDim2.new(0, 0, 0, H_HEADER + 4)
    ...
    local function makeActionBtn(labelText, isAccent, onClick)
        local btn = Instance.new("TextButton", body)
        btn.Size             = UDim2.new(1, -PAD*2, 0, BTN_H)
        btn.Position         = UDim2.new(0, PAD, 0, yOffset)
        btn.BackgroundColor3 = isAccent and QA.BtnAccent or QA.BtnBase
        btn.BorderSizePixel  = 0
        btn.Font             = Enum.Font.GothamBold
        btn.TextSize         = 10
        btn.TextColor3       = isAccent and Color3.new(1,1,1) or QA.TxtMain
        btn.Text             = labelText
        btn.AutoButtonColor  = false
        corner(btn, 5)
        if not isAccent then
            stroke(btn, QA.Divider, 1, 0)
        end

        -- Hover effect
        btn.MouseEnter:Connect(function()
            btn.BackgroundColor3 = isAccent and QA.BtnAccentHv or QA.BtnHover
        end)
        btn.MouseLeave:Connect(function()
            btn.BackgroundColor3 = isAccent and QA.BtnAccent or QA.BtnBase
        end)
        ...
        yOffset = yOffset + BTN_H + 4
        return btn
    end

    -- ── Fábrica de toggle ─────────────────────────────────────────
    local function makeToggleRow(labelText, getState, onToggle)
        -- Row
        local row = Instance.new("Frame", body)
        row.Size             = UDim2.new(1, -PAD*2, 0, BTN_H)
        row.Position         = UDim2.new(0, PAD, 0, yOffset)
        row.BackgroundColor3 = QA.BtnBase
        row.BorderSizePixel  = 0
        corner(row, 5)
        stroke(row, QA.Divider, 1, 0)
        ...
        local function refreshToggle()
            local on = getState()
            pill.BackgroundColor3 = on and QA.ToggleOn or QA.ToggleOff
            TweenService2:Create(knob, TweenInfo.new(0.12), {
                Position = on
        ...
        -- Clique na row inteira
        local clickTarget = Instance.new("TextButton", row)
        clickTarget.Size              = UDim2.new(1, 0, 1, 0)
        clickTarget.BackgroundTransparency = 1
        clickTarget.Text              = ""
        clickTarget.ZIndex            = row.ZIndex + 2
        clickTarget.MouseButton1Click:Connect(doToggle)
        clickTarget.InputBegan:Connect(function(inp)
            if inp.UserInputType == Enum.UserInputType.Touch then doToggle() end
        end)

        yOffset = yOffset + BTN_H + 4
        return refreshToggle  -- retorna função para atualização externa
    end
    ...
    local function makeDivider()
        ...
        d.Position         = UDim2.new(0, PAD, 0, yOffset)
        d.BackgroundColor3 = QA.Divider
        d.BorderSizePixel  = 0
        yOffset = yOffset + 5
    end
    ...
    -- 1. Instant Respawn
    makeActionBtn("⟳  RESPAWN INSTANT", false, function()
        local char = LocalPlayer.Character
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if hum then
            hum.Health = 0
            ShowNotification("QUICK ACTIONS", "Respawnando...")
        end
    end)

    -- 2. Instant Clone
    makeActionBtn("⬡  CLONAR INSTANT", false, function()
        ShowNotification("QUICK ACTIONS", "Clonando...")
        task.spawn(function()
            if instantClone then
                instantClone()
            elseif _G.instantClone then
                _G.instantClone()
            else
                ShowNotification("CLONE", "Sem Quantum Cloner!")
            end
        end)
    end)
    ...
    -- 3. Rejoin Server
    makeActionBtn("↺  REJOIN SERVIDOR", false, function()
        ShowNotification("QUICK ACTIONS", "Reconnecting...")
        task.delay(0.5, function()
            pcall(function()
    ...
    -- 4. Kick Self
    makeActionBtn("✕  KICK SELF", true, function()
        ShowNotification("QUICK ACTIONS", "Saindo...")
        task.delay(0.3, function()
            pcall(function() LocalPlayer:Kick("\nWS Hub — Kick Self") end)
        end)
    end)
    ...
    -- 5. TP Highest Gen
    makeActionBtn("▲  TP MAIOR GEN", false, function()
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if not hrp then ShowNotification("QUICK ACTIONS","Sem personagem!"); return end

        -- Tenta encontrar o plot com maior geração via Workspace
        local bestPos, bestGen = nil, -1
        local plots = Workspace:FindFirstChild("Plots")
        if plots then
            for _, plot in ipairs(plots:GetChildren()) do
                -- Tenta ler Generation ou Cash do plot
                local gen = 0
                local genVal = plot:FindFirstChild("Generation") or plot:FindFirstChild("Gen")
                if genVal and genVal:IsA("NumberValue") or (genVal and genVal:IsA("IntValue")) then
                    gen = genVal.Value or 0
                end
                if gen > bestGen then
                    bestGen = gen
                    ...
                    if ap then bestPos = ap.Position + Vector3.new(0, 6, 0)
                    elseif dh then bestPos = dh.Position + Vector3.new(0, 6, 0)
                    elseif pp then bestPos = pp.Position + Vector3.new(0, 6, 0)
                    end
                end
            end
        end
        ...
        if bestPos then
            ...
            ShowNotification("QUICK ACTIONS", "TP → Maior Gen: " .. bestGen)
        else
            -- fallback: SharedState.BestConveyorGv
            ShowNotification("QUICK ACTIONS", "Nenhum plot encontrado!")
        end
    end)
    ...
    -- Toggle Auto-Kick After Steal
    local refreshAK = makeToggleRow(
        "Auto-Kick p/ Steal",
        function() return qaState.autoKick end,
        function(val)
            qaState.autoKick = val
            if _G.setAutoKickFromSettings then
                _G.setAutoKickFromSettings(val)
            else
                Config.AutoKickOnSteal = val
                SaveConfig()
            end
            ShowNotification("AUTO-KICK", val and "ATIVADO" or "DESATIVADO")
        end
    )

    -- Sincroniza com o painel principal se atualizado externamente
    if _G.setAutoKickFromMiniUI then
        -- já existe outro mini UI registrado, encadeamos
        local prev = _G.setAutoKickFromMiniUI
        _G.setAutoKickFromMiniUI = function()
            qaState.autoKick = Config.AutoKickOnSteal
            refreshAK()
            prev()
        end
    else
    ...
    -- Toggle Auto Hop (InfiniteJump)
    local refreshHop = makeToggleRow(
        "Auto Hop (Inf. Jump)",
        function() return qaState.autoHop end,
        function(val)
            qaState.autoHop = val
            if setInfiniteJump then
                setInfiniteJump(val)
            elseif _G.setInfiniteJump then
                _G.setInfiniteJump(val)
            else
                Config.TpSettings.InfiniteJump = val
                SaveConfig()
            end
            ShowNotification("AUTO HOP", val and "ATIVADO" or "DESATIVADO")
        end
    )
    ...
    do
        ...
        header.InputEnded:Connect(function(inp)
            ...
            or inp.UserInputType == Enum.UserInputType.Touch then
                dragging = false
                -- Salva posição no config
                if Config and Config.Positions then
                    Config.Positions.QuickActions = {
                        X = root.Position.X.Scale,
                        Y = root.Position.Y.Scale,
                    }
                    SaveConfig()
                end
            end
    ...
    minBtn.MouseButton1Click:Connect(function()
        qaState.minimized = not qaState.minimized
        local targetH = qaState.minimized and MIN_HEIGHT or FULL_HEIGHT
        local targetText = qaState.minimized and "+" or "─"
        minBtn.Text = targetText
        body.Visible = not qaState.minimized

        game:GetService("TweenService"):Create(root,
            TweenInfo.new(0.18, Enum.EasingStyle.Quad),
            { Size = UDim2.new(0, W, 0, targetH + 2) }
        ):Play()
    end)
    ...
    -- ── Inicialização dos toggles com estado atual ─────────────
    qaState.autoKick = Config and Config.AutoKickOnSteal or false
    qaState.autoHop  = Config and Config.TpSettings and Config.TpSettings.InfiniteJump or false
    refreshAK()
    refreshHop()

    -- Adiciona "QuickActions" ao mapa de posições do config default
    if Config and Config.Positions and not Config.Positions.QuickActions then
        Config.Positions.QuickActions = { X = 0.78, Y = 0.22 }
        SaveConfig()
    end

    -- Expõe globalmente para integração com outros painéis
    _G.QA_refreshAK  = refreshAK
    _G.QA_refreshHop = refreshHop

end -- fim QuickActions Panel
...
task.spawn(function()
    ...
    f.Position = UDim2.new(0.5, -100, 0.5, -30)
