...
local function setInfiniteJump(enabled)
    ...
    if Connections._ijJumpRequest then Connections._ijJumpRequest:Disconnect(); Connections._ijJumpRequest = nil end
    infiniteJumpConnection = Connections.infiniteJumpConnection
    if not enabled then return end

    -- JumpRequest dispara no botao mobile E no Space do teclado — funciona nos dois
    local jumpReqConn = UserInputService.JumpRequest:Connect(function()
        ...
        -- Forca pulo extra quando estiver no ar
    ...
    Connections._ijJumpRequest = jumpReqConn
    Connections.infiniteJumpConnection = jumpReqConn
    infiniteJumpConnection = jumpReqConn
end
...
task.spawn(function()
    ...
    local function isMyBaseAnimal(animalData)
        ...
        -- Metodo 1: Synchronizer
        local ok, ch = pcall(function() return Synchronizer:Get(animalData.plot) end)
        if ok and ch then
        ...
        -- Metodo 2: PlotSign YourBase (fallback confiavel)
        local sign = plot:FindFirstChild("PlotSign")
        local yb   = sign and sign:FindFirstChild("YourBase")
        if yb and yb:IsA("BillboardGui") and yb.Enabled then return true end
        -- Metodo 3: owner salvo no animalData
        if animalData.owner then
            return animalData.owner == LocalPlayer.Name
                or animalData.owner == tostring(LocalPlayer.UserId)
        end
        return false
    end
    ...
    local function scanSinglePlot(plot)
        ...
        pcall(function()
            ...
            local owner = ch:Get("Owner")
            -- Aceita plots de donos offline tambem (animais ficam na base mesmo sem o dono)
            -- Extrai ownerName de Instance ou table
            local ownerName = "Unknown"
            if owner then
                if typeof(owner) == "Instance" and owner:IsA("Player") then
                    ownerName = owner.Name
                elseif type(owner) == "table" then
                    ownerName = owner.Name or owner.Username or tostring(owner.UserId or "Unknown")
                end
            end
...
local function runAutoSnipe()
    ...
    if hrp.Position.Y > 25 and targetPosition.Y > 25 then
        ...
        hrp.AssemblyLinearVelocity = Vector3.zero
        hrp.AssemblyAngularVelocity = Vector3.zero
        -- Segura a posição por 350ms pra evitar lagback do servidor
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    if input.KeyCode == cloneKey then
        ...
        -- Verifica se o player está na própria base
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        ...
        if inMyBase then
            -- Dentro da base: clona e vai pro melhor brainrot instantaneamente
            task.spawn(function()
                instantClone()
                while _G.isCloning do task.wait() end
...
function buildBullysSettingsUI()
    ...
    -- ── DESYNC TAB ──────────────────────────────────────────────────
    local dsyS = tScrolls2["dsy"]
    makeSec(dsyS, "DESYNC RAKNET", 0)
    do
        -- Toggle principal
        local dsyRow = Instance.new("Frame", dsyS)
        dsyRow.Size = UDim2.new(1,0,0,36)
        dsyRow.BackgroundColor3 = C().SURF
        dsyRow.BorderSizePixel = 0
        dsyRow.LayoutOrder = 1
        Instance.new("UICorner", dsyRow).CornerRadius = UDim.new(0,7)

        local dsyLbl = Instance.new("TextLabel", dsyRow)
        dsyLbl.Size = UDim2.new(1,-54,1,0)
        dsyLbl.Position = UDim2.new(0,10,0,0)
        dsyLbl.BackgroundTransparency = 1
        dsyLbl.Text = "Desync RakNet"
        dsyLbl.Font = Enum.Font.GothamBold
        dsyLbl.TextSize = 12
        dsyLbl.TextColor3 = C().TP
        dsyLbl.TextXAlignment = Enum.TextXAlignment.Left

        local dsySw = Instance.new("Frame", dsyRow)
        dsySw.Size = UDim2.new(0,38,0,20)
        dsySw.Position = UDim2.new(1,-46,0.5,-10)
        dsySw.BackgroundColor3 = Config.RaknetDesyncEnabled and Theme.Accent1 or C().SH
        Instance.new("UICorner", dsySw).CornerRadius = UDim.new(1,0)

        local dsyDot = Instance.new("Frame", dsySw)
        dsyDot.Size = UDim2.new(0,15,0,15)
        dsyDot.Position = Config.RaknetDesyncEnabled and UDim2.new(1,-17,0.5,-7.5) or UDim2.new(0,2,0.5,-7.5)
        dsyDot.BackgroundColor3 = Color3.new(1,1,1)
        Instance.new("UICorner", dsyDot).CornerRadius = UDim.new(1,0)

        local dsyBtn = Instance.new("TextButton", dsySw)
        dsyBtn.Size = UDim2.new(1,0,1,0)
        dsyBtn.BackgroundTransparency = 1
        dsyBtn.Text = ""; dsyBtn.ZIndex = 4; dsyBtn.AutoButtonColor = false

        local function toggleDsy()
            local ns = not _G.getRaknetDesync()
            _G.setRaknetDesync(ns)
            TweenService:Create(dsyDot, TweenInfo.new(0.15), {
                Position = ns and UDim2.new(1,-17,0.5,-7.5) or UDim2.new(0,2,0.5,-7.5)
            }):Play()
            TweenService:Create(dsySw, TweenInfo.new(0.15), {
                BackgroundColor3 = ns and Theme.Accent1 or C().SH
            }):Play()
        end
        dsyBtn.MouseButton1Click:Connect(toggleDsy)
        dsyBtn.InputBegan:Connect(function(i)
            if i.UserInputType == Enum.UserInputType.Touch then toggleDsy() end
        end)

        -- Status label
        local dsyInfo = Instance.new("Frame", dsyS)
        dsyInfo.Size = UDim2.new(1,0,0,52)
        dsyInfo.BackgroundColor3 = C().SURF
        dsyInfo.BorderSizePixel = 0
        dsyInfo.LayoutOrder = 2
        Instance.new("UICorner", dsyInfo).CornerRadius = UDim.new(0,7)

        local dsyDesc = Instance.new("TextLabel", dsyInfo)
        ...
        dsyDesc.TextXAlignment = Enum.TextXAlignment.Left
    ...
    do
        ...
        -- Toggle Smart Sell
        local sellToggleRow = Instance.new("Frame", sellS)
        sellToggleRow.Size = UDim2.new(1,0,0,36)
        sellToggleRow.BackgroundColor3 = C().SURF
        sellToggleRow.BorderSizePixel = 0
        sellToggleRow.LayoutOrder = 1
        Instance.new("UICorner", sellToggleRow).CornerRadius = UDim.new(0,7)

        local sellLbl = Instance.new("TextLabel", sellToggleRow)
        sellLbl.Size = UDim2.new(1,-54,1,0)
        sellLbl.Position = UDim2.new(0,10,0,0)
        sellLbl.BackgroundTransparency = 1
        sellLbl.Text = "Smart Sell"
        sellLbl.Font = Enum.Font.GothamBold
        sellLbl.TextSize = 12
        sellLbl.TextColor3 = C().TP
        sellLbl.TextXAlignment = Enum.TextXAlignment.Left

        local sellSw = Instance.new("Frame", sellToggleRow)
        sellSw.Size = UDim2.new(0,38,0,20)
        sellSw.Position = UDim2.new(1,-46,0.5,-10)
        sellSw.BackgroundColor3 = Config.SmartSellEnabled and Theme.Accent1 or C().SH
        Instance.new("UICorner", sellSw).CornerRadius = UDim.new(1,0)
        local sellDot = Instance.new("Frame", sellSw)
        sellDot.Size = UDim2.new(0,15,0,15)
        sellDot.Position = Config.SmartSellEnabled and UDim2.new(1,-17,0.5,-7.5) or UDim2.new(0,2,0.5,-7.5)
        sellDot.BackgroundColor3 = Color3.new(1,1,1)
        Instance.new("UICorner", sellDot).CornerRadius = UDim.new(1,0)
        local sellSwBtn = Instance.new("TextButton", sellSw)
        sellSwBtn.Size = UDim2.new(1,0,1,0)
        sellSwBtn.BackgroundTransparency = 1
        sellSwBtn.Text = ""; sellSwBtn.ZIndex = 4; sellSwBtn.AutoButtonColor = false

        local function toggleSmartSell()
            Config.SmartSellEnabled = not Config.SmartSellEnabled
            SaveConfig()
            TweenService:Create(sellDot, TweenInfo.new(0.15), {
                Position = Config.SmartSellEnabled and UDim2.new(1,-17,0.5,-7.5) or UDim2.new(0,2,0.5,-7.5)
            }):Play()
            TweenService:Create(sellSw, TweenInfo.new(0.15), {
                BackgroundColor3 = Config.SmartSellEnabled and Theme.Accent1 or C().SH
            }):Play()
        end
        sellSwBtn.MouseButton1Click:Connect(toggleSmartSell)
        sellSwBtn.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch then toggleSmartSell() end end)

        -- Valor mínimo
        local minValRow = Instance.new("Frame", sellS)
        minValRow.Size = UDim2.new(1,0,0,36)
        minValRow.BackgroundColor3 = C().SURF
        minValRow.BorderSizePixel = 0
        minValRow.LayoutOrder = 2
        Instance.new("UICorner", minValRow).CornerRadius = UDim.new(0,7)

        local minValLbl = Instance.new("TextLabel", minValRow)
        minValLbl.Size = UDim2.new(0.55,0,1,0)
        minValLbl.Position = UDim2.new(0,10,0,0)
        minValLbl.BackgroundTransparency = 1
        minValLbl.Text = "Valor mínimo ($/s)"
        minValLbl.Font = Enum.Font.GothamBold
        minValLbl.TextSize = 11
        minValLbl.TextColor3 = C().TP
        minValLbl.TextXAlignment = Enum.TextXAlignment.Left

        local minValBox = Instance.new("TextBox", minValRow)
        minValBox.Size = UDim2.new(0,90,0,24)
        minValBox.Position = UDim2.new(1,-100,0.5,-12)
        minValBox.BackgroundColor3 = C().BG
        minValBox.BorderSizePixel = 0
        minValBox.Font = Enum.Font.GothamBold
        minValBox.TextSize = 12
        minValBox.TextColor3 = C().TP
        minValBox.Text = tostring(Config.SmartSellMinValue)
        minValBox.PlaceholderText = "0"
        minValBox.TextXAlignment = Enum.TextXAlignment.Center
        Instance.new("UICorner", minValBox).CornerRadius = UDim.new(0,6)
        minValBox.FocusLost:Connect(function()
            local v = tonumber(minValBox.Text) or 0
            Config.SmartSellMinValue = v
            minValBox.Text = tostring(v)
            SaveConfig()
        end)

        -- Raridades selecionáveis
        makeSec(sellS, "RARIDADES PARA VENDER", 3)

        local function isRaritySelected(rar)
            for _, r in ipairs(Config.SmartSellRarities) do
                if r == rar then return true end
            end
            return false
        end
        ...
        for i, rar in ipairs(RARITIES) do
            local rarRow = Instance.new("Frame", sellS)
            rarRow.Size = UDim2.new(1,0,0,32)
            rarRow.BackgroundColor3 = C().SURF
            rarRow.BorderSizePixel = 0
            rarRow.LayoutOrder = 3 + i
            Instance.new("UICorner", rarRow).CornerRadius = UDim.new(0,7)

            local rarLbl = Instance.new("TextLabel", rarRow)
            rarLbl.Size = UDim2.new(1,-54,1,0)
            rarLbl.Position = UDim2.new(0,10,0,0)
            rarLbl.BackgroundTransparency = 1
            rarLbl.Text = rar
            rarLbl.Font = Enum.Font.GothamBold
            rarLbl.TextSize = 12
            rarLbl.TextColor3 = RARITY_COLORS[rar] or C().TP
            rarLbl.TextXAlignment = Enum.TextXAlignment.Left

            local sel = isRaritySelected(rar)
            local rarChk = Instance.new("TextButton", rarRow)
            rarChk.Size = UDim2.new(0,26,0,26)
            rarChk.Position = UDim2.new(1,-36,0.5,-13)
            rarChk.BackgroundColor3 = sel and (RARITY_COLORS[rar] or Theme.Accent1) or C().SH
            rarChk.BorderSizePixel = 0
            rarChk.Font = Enum.Font.GothamBlack
            rarChk.TextSize = 14
            rarChk.TextColor3 = Color3.new(1,1,1)
            rarChk.Text = sel and "✓" or ""
            rarChk.AutoButtonColor = false
            Instance.new("UICorner", rarChk).CornerRadius = UDim.new(0,6)
            rarBtns[rar] = rarChk

            local function toggleRar()
                local on = isRaritySelected(rar)
                if on then
                    local newList = {}
                    for _, r in ipairs(Config.SmartSellRarities) do
                        if r ~= rar then table.insert(newList, r) end
                    end
                    Config.SmartSellRarities = newList
                else
                    table.insert(Config.SmartSellRarities, rar)
                end
                SaveConfig()
                local nowOn = isRaritySelected(rar)
                TweenService:Create(rarChk, TweenInfo.new(0.12), {
                    BackgroundColor3 = nowOn and (RARITY_COLORS[rar] or Theme.Accent1) or C().SH
                }):Play()
                rarChk.Text = nowOn and "✓" or ""
            end
            rarChk.MouseButton1Click:Connect(toggleRar)
            rarChk.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.Touch then toggleRar() end end)
        end

        -- Botão Sell Now
        makeSec(sellS, "AÇÃO", 12)
        local sellNowBtn = Instance.new("TextButton", sellS)
        sellNowBtn.Size = UDim2.new(1,0,0,36)
        sellNowBtn.BackgroundColor3 = Color3.fromRGB(50,50,50)
        sellNowBtn.BorderSizePixel = 0
        sellNowBtn.Font = Enum.Font.GothamBlack
        sellNowBtn.TextSize = 13
        sellNowBtn.TextColor3 = Color3.fromRGB(210,210,210)
        sellNowBtn.Text = "VENDER AGORA"
        sellNowBtn.AutoButtonColor = false
        sellNowBtn.LayoutOrder = 13
        Instance.new("UICorner", sellNowBtn).CornerRadius = UDim.new(0,7)
        Instance.new("UIStroke", sellNowBtn).Color = Color3.fromRGB(80,80,80)
        ...
        local function doSmartSell()
            ...
            -- Buscar AnimalsShared
            local ok, AnimSell = pcall(function()
                return require(ReplicatedStorage:WaitForChild("Shared"):WaitForChild("Animals"))
            end)
            if not ok then return end

            -- Buscar podiums do meu plot
            local Plots = Workspace:FindFirstChild("Plots")
            if not Plots then return end
            local myPlot = nil
            for _, plot in ipairs(Plots:GetChildren()) do
                ...
                if att then
                    for _, pp in ipairs(att:GetChildren()) do
                        if pp:IsA("ProximityPrompt") and pp.Enabled then
                            -- Descobrir raridade/valor do animal neste podium
                            local animal = pod:FindFirstChildOfClass("Model") or pod:FindFirstChild("Animal")
                            local animalName = animal and animal.Name or ""
                            local gv = 0
                            pcall(function()
                                local idx = pod:GetAttribute("Index") or animalName
                                local mut = pod:GetAttribute("Mutation")
                                gv = AnimSell:GetGeneration(idx, mut, nil, nil) or 0
                            end)
                            ...
                            pcall(function()
                                local Datas = ReplicatedStorage:WaitForChild("Datas")
                                local AD = require(Datas:WaitForChild("Animals"))
                                local d = AD[animalName] or AD[pod:GetAttribute("Index") or ""]
                                if d then rar = d.Rarity end
                            end)

                            local passValue = Config.SmartSellMinValue <= 0 or gv >= Config.SmartSellMinValue
                            local passRarity = #Config.SmartSellRarities == 0 or (rar and isRaritySelected(rar))

                            if passValue and passRarity then
                                local dist = (hrp.Position - att.WorldPosition).Magnitude
                                if dist > 10 then
                                    hrp.CFrame = CFrame.new(att.WorldPosition + Vector3.new(0,3,0))
                                    task.wait(0.1)
                                end
                                pcall(function()
                                    fireclickdetector = fireclickdetector or function() end
                                    pp.Triggered:Fire(LocalPlayer)
                                end)
                                pcall(function() pp:FirePromptButtonClickEvent(LocalPlayer) end)
                                sold = sold + 1
                                task.wait(0.3)
                            end
                        end
        ...
        sellNowBtn.MouseButton1Click:Connect(function()
            TweenService:Create(sellNowBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(70,70,70)}):Play()
            task.delay(0.15, function() TweenService:Create(sellNowBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(50,50,50)}):Play() end)
...
function buildMiniActionsUI()
    ...
    -- ── REMOTE SELL ──────────────────────────────────────────────────────────
    -- ── MOVIMENTO ──────────────────────────────────────────────────────────
    miniSec("MOVIMENTO", 50)

    -- Carpet Speed toggle (mobile pode acionar sem keybind)
    local carpetMiniBtn = mBtn("Carpet Speed: OFF", 51, Theme.Surface, function() end)
    local function updCarpetMini()
        local on = State.carpetSpeedEnabled
        carpetMiniBtn.Text             = on and "Carpet Speed: ON"  or "Carpet Speed: OFF"
        carpetMiniBtn.BackgroundColor3 = on and Theme.Accent1       or Theme.Surface
        carpetMiniBtn.TextColor3       = on and Color3.new(0,0,0)   or Theme.TextPrimary
    end
    updCarpetMini()
    carpetMiniBtn.MouseButton1Click:Connect(function()
        local newState = not State.carpetSpeedEnabled
        setCarpetSpeed(newState)
        if _carpetStatusLabel then
            _carpetStatusLabel.Text       = newState and "ON" or "OFF"
        ...
        updCarpetMini()
        ShowNotification("CARPET SPEED", newState and ("ON  |  "..Config.TpSettings.Tool.."  |  140") or "OFF")
    end)
    -- Sincroniza quando o estado muda pelo keybind
    task.spawn(function()
        while carpetMiniBtn and carpetMiniBtn.Parent do
            updCarpetMini()
            task.wait(0.3)
        end
    end)
    ...
    local ijMiniBtn = mBtn("Infinite Jump: OFF", 52, Theme.Surface, function() end)
    local function updIJMini()
        local on = State.infiniteJumpEnabled
        ijMiniBtn.Text             = on and "Infinite Jump: ON"  or "Infinite Jump: OFF"
        ijMiniBtn.BackgroundColor3 = on and Theme.Accent1        or Theme.Surface
        ijMiniBtn.TextColor3       = on and Color3.new(0,0,0)    or Theme.TextPrimary
    end
    updIJMini()
    ijMiniBtn.MouseButton1Click:Connect(function()
    ...
    local rsMainFrame  = nil
    local rsScanFn     = nil

    do
        local old = PlayerGui:FindFirstChild("NHRemoteSellGUI")
        if old then old:Destroy() end

        local rsGui = Instance.new("ScreenGui")
        rsGui.Name         = "NHRemoteSellGUI"
        rsGui.ResetOnSpawn = false
        rsGui.DisplayOrder = 28
        rsGui.Parent       = PlayerGui

        local RS_W, RS_H = 280, 460
        local rsScreen   = workspace.CurrentCamera.ViewportSize

        local function rsClamp(x, y)
            return math.clamp(x, 0, rsScreen.X - RS_W),
                   math.clamp(y, 0, rsScreen.Y - RS_H)
        end

        local RS_SAVE = "notzhub_sell_pos.txt"
        local svX, svY = nil, nil
        local pok, pdata = pcall(readfile, RS_SAVE)
        if pok and pdata then
            local x, y = pdata:match("(-?[%d%.]+),(-?[%d%.]+)")
            if x and y then svX, svY = rsClamp(tonumber(x), tonumber(y)) end
        end

        -- ── Main frame ────────────────────────────────────────────────────
        local mf = Instance.new("Frame")
        mf.Name              = "RSFrame"
        mf.Size              = UDim2.new(0, RS_W, 0, RS_H)
        mf.Position          = (svX and svY)
                               and UDim2.new(0, svX, 0, svY)
                               or  UDim2.new(0.5, -(RS_W/2), 0.5, -(RS_H/2))
        ...
        do
            ...
            mf.InputBegan:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.MouseButton1 then
                    drag = true; dragStart = inp.Position; startPos = mf.Position
                end
            end)
            UserInputService.InputChanged:Connect(function(inp)
                if drag and inp.UserInputType == Enum.UserInputType.MouseMovement then
                    local d  = inp.Position - dragStart
                    local cx, cy = rsClamp(startPos.X.Offset + d.X, startPos.Y.Offset + d.Y)
                    mf.Position = UDim2.new(0, cx, 0, cy)
                end
            end)
            UserInputService.InputEnded:Connect(function(inp)
                if inp.UserInputType == Enum.UserInputType.MouseButton1 and drag then
                    drag = false
                    pcall(writefile, RS_SAVE,
                        mf.Position.X.Offset .. "," .. mf.Position.Y.Offset)
                end
            end)
        end
        ...
        -- Minimize btn
        local minBtn2 = Instance.new("TextButton", hdr)
        minBtn2.Size             = UDim2.new(0, 26, 0, 26)
        minBtn2.Position         = UDim2.new(1, -34, 0.5, -13)
        minBtn2.BackgroundTransparency = 1
        minBtn2.Text             = "−"
        minBtn2.TextColor3       = Theme.TextPrimary
        ...
        -- Title
        local rsTitle = Instance.new("TextLabel", hdr)
        ...
        -- ── Sell All btn ──────────────────────────────────────────────────
        local rsSellAll = Instance.new("TextButton", mf)
        rsSellAll.Size             = UDim2.new(1, -80, 0, 32)
        rsSellAll.Position         = UDim2.new(0, 9, 0, 58)
        rsSellAll.BackgroundColor3 = Theme.Accent1
        rsSellAll.Text             = "SELL ALL"
        rsSellAll.TextColor3       = Color3.new(1, 1, 1)
        rsSellAll.Font             = Enum.Font.GothamBlack
        rsSellAll.TextSize         = 12
        rsSellAll.BorderSizePixel  = 0
        rsSellAll.ZIndex           = 3
        Instance.new("UICorner", rsSellAll).CornerRadius = UDim.new(0, 8)

        -- ── Scan btn (right of Sell All) ─────────────────────────────────
        local rsScanBtn = Instance.new("TextButton", mf)
        rsScanBtn.Size             = UDim2.new(0, 60, 0, 32)
        rsScanBtn.Position         = UDim2.new(1, -70, 0, 58)
        rsScanBtn.BackgroundColor3 = Theme.SurfaceHighlight
        rsScanBtn.Text             = "↺ SCAN"
        rsScanBtn.TextColor3       = Theme.Accent1
        rsScanBtn.Font             = Enum.Font.GothamBold
        rsScanBtn.TextSize         = 10
        rsScanBtn.BorderSizePixel  = 0
        rsScanBtn.ZIndex           = 3
        Instance.new("UICorner", rsScanBtn).CornerRadius = UDim.new(0, 8)
        local rsScanStroke = Instance.new("UIStroke", rsScanBtn)
        rsScanStroke.Color = Theme.Accent1; rsScanStroke.Thickness = 1; rsScanStroke.Transparency = 0.4

        -- ── Status label ─────────────────────────────────────────────────
        local rsStatus = Instance.new("TextLabel", mf)
        rsStatus.Size             = UDim2.new(1, -18, 0, 20)
        rsStatus.Position         = UDim2.new(0, 9, 0, 96)
        rsStatus.BackgroundTransparency = 1
        rsStatus.Text             = "Clique SCAN para carregar"
        rsStatus.TextColor3       = Theme.TextSecondary
        rsStatus.Font             = Enum.Font.Gotham
        rsStatus.TextSize         = 10
        rsStatus.TextXAlignment   = Enum.TextXAlignment.Left
        rsStatus.ZIndex           = 3

        -- ── Scroll list ──────────────────────────────────────────────────
        local rsScroll = Instance.new("ScrollingFrame", mf)
        rsScroll.Size                  = UDim2.new(1, -18, 1, -148)
        rsScroll.Position              = UDim2.new(0, 9, 0, 120)
        rsScroll.BackgroundColor3      = Theme.Surface
        rsScroll.BackgroundTransparency = 0.1
        rsScroll.BorderSizePixel       = 0
        rsScroll.ScrollBarThickness    = 3
        rsScroll.ScrollBarImageColor3  = Theme.Accent1
        ...
        rsLayout.Padding   = UDim.new(0, 5)
        local rsPad = Instance.new("UIPadding", rsScroll)
        rsPad.PaddingTop    = UDim.new(0, 6); rsPad.PaddingBottom = UDim.new(0, 6)
        ...
        rsLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            rsScroll.CanvasSize = UDim2.new(0, 0, 0, rsLayout.AbsoluteContentSize.Y + 12)
        end)

        -- ── Minimize logic ────────────────────────────────────────────────
        local rsMinimized = false
        local function toggleRS_Minimize()
            rsMinimized = not rsMinimized
            if rsMinimized then
                mf.Size         = UDim2.new(0, RS_W, 0, 50)
                rsSellAll.Visible   = false
                rsScanBtn.Visible   = false
                rsStatus.Visible    = false
                rsScroll.Visible    = false
                accentLine.Visible  = false
                minBtn2.Text        = "+"
            else
                mf.Size         = UDim2.new(0, RS_W, 0, RS_H)
                rsSellAll.Visible   = true
                rsScanBtn.Visible   = true
                rsStatus.Visible    = true
                rsScroll.Visible    = true
                accentLine.Visible  = true
                minBtn2.Text        = "−"
            end
        end
        minBtn2.MouseButton1Click:Connect(toggleRS_Minimize)

        -- ── Mutation color map ────────────────────────────────────────────
        local RS_MUT = {
            Rainbow    = Color3.fromRGB(100, 200, 255),
            Gold       = Color3.fromRGB(255, 200, 50),
            Diamond    = Color3.fromRGB(120, 220, 255),
            Cursed     = Color3.fromRGB(180, 60,  60),
            Candy      = Color3.fromRGB(255, 150, 200),
            Divine     = Color3.fromRGB(220, 220, 255),
            YinYang    = Color3.fromRGB(200, 200, 200),
            Lava       = Color3.fromRGB(255, 120, 30),
            Bloodrot   = Color3.fromRGB(180, 30,  30),
            Radioactive= Color3.fromRGB(120, 220, 60),
        }
        local function rsMutCol(m)
            if not m then return Theme.TextSecondary end
            return RS_MUT[m] or Theme.TextSecondary
        end

        -- ── Game modules (async) ─────────────────────────────────────────
        local rsAD, rsAS, rsNU, rsSY = nil, nil, nil, nil
        task.spawn(function()
            pcall(function()
                local D = ReplicatedStorage:WaitForChild("Datas",   10)
                local S = ReplicatedStorage:WaitForChild("Shared",  10)
                local U = ReplicatedStorage:WaitForChild("Utils",   10)
                local P = ReplicatedStorage:WaitForChild("Packages",10)
                if D then rsAD = require(D:WaitForChild("Animals"))           end
                if S then rsAS = require(S:WaitForChild("Animals"))           end
                if U then rsNU = require(U:WaitForChild("NumberUtils"))       end
                if P then rsSY = require(P:WaitForChild("Synchronizer"))      end
            end)
        end)

        -- ── Find my plot ──────────────────────────────────────────────────
        local rsLockedPlot = nil
        local function rsFindPlot()
            if rsLockedPlot and rsLockedPlot.Parent then return rsLockedPlot end
            local plots = workspace:FindFirstChild("Plots")
            if not plots then return nil end
            local myName = LocalPlayer.Name:lower()
            local myId   = tostring(LocalPlayer.UserId)
            local myDN   = LocalPlayer.DisplayName:lower()
            for _, plot in ipairs(plots:GetChildren()) do
                if rsSY then
                    local ok, ch = pcall(function() return rsSY:Get(plot.Name) end)
                    if ok and ch then
                        local owner = ch:Get("Owner")
                        if owner then
                            if typeof(owner)=="Instance" and owner:IsA("Player") and owner==LocalPlayer then
                                rsLockedPlot=plot; return plot
                ...
                -- YourBase BillboardGui (método usado pelo resto do hub)
                local sign = plot:FindFirstChild("PlotSign")
                if sign then
                    local yb = sign:FindFirstChild("YourBase")
                    if yb and yb:IsA("BillboardGui") and yb.Enabled then
        ...
        local function rsGetData(plot, slotKey)
            if rsSY then
                local ok, ch = pcall(function() return rsSY:Get(plot.Name) end)
                if ok and ch then
                    local al = ch:Get("AnimalList")
                    if type(al)=="table" then
                        ...
                        if type(entry)=="table" and entry.Index then
                            local aInfo = rsAD and rsAD[entry.Index]
                            local dName = (aInfo and aInfo.DisplayName) or entry.Index
                            local gv, gt = 0, ""
                            if rsAS then
                                pcall(function()
                                    gv = rsAS:GetGeneration(entry.Index, entry.Mutation, entry.Traits, nil)
                                end)
                            end
                            if rsNU and gv>0 then gt = "$"..rsNU:ToString(gv).."/s" end
                            local mut = entry.Mutation
                            if mut=="None" or mut=="" then mut=nil end
                            return { name=dName, genText=gt, genVal=gv, mutation=mut }
                        end
                    end
                end
            end
            -- Fallback: billboard scan
            local podiums = plot:FindFirstChild("AnimalPodiums")
            if podiums then
                ...
                if podium then
                    ...
                    local spawn = base and base:FindFirstChild("Spawn")
                    if spawn then
                        for _, bb in ipairs(spawn:GetDescendants()) do
                        ...
                        for _, obj in ipairs(podium:GetDescendants()) do
                            ...
                            and obj.Name~="Base" and obj.Name~="Spawn" then
                                local dn=(rsAD and rsAD[obj.Name] and rsAD[obj.Name].DisplayName) or obj.Name
        ...
        local function rsAddRow(podNum, sellPrompt, info)
            local name    = info and info.name    or ("Slot "..podNum)
            local genText = info and info.genText or ""
            local mut     = info and info.mutation
            local ac      = rsMutCol(mut)

            local row = Instance.new("Frame", rsScroll)
            row.Size             = UDim2.new(1, -12, 0, 50)
            row.BackgroundColor3 = Theme.SurfaceHighlight
            row.BorderSizePixel  = 0
            row.LayoutOrder      = podNum
            row.ZIndex           = 4
            Instance.new("UICorner", row).CornerRadius = UDim.new(0, 8)
            local rS = Instance.new("UIStroke", row)
            rS.Color = Theme.Accent2; rS.Thickness=1; rS.Transparency=0.55

            -- Colored mutation bar
            local bar = Instance.new("Frame", row)
            ...
            if mut or genText ~= "" then
                ...
                subLbl.TextColor3 = ac
            else
                subLbl.Text      = "—"
                subLbl.TextColor3 = Theme.TextSecondary
            end

            -- Sell button (NOTZHUB red style)
            local sellBtn = Instance.new("TextButton", row)
            sellBtn.Size             = UDim2.new(0, 56, 0, 30)
            sellBtn.Position         = UDim2.new(1, -62, 0.5, -15)
            sellBtn.BackgroundColor3 = Theme.Accent1
            sellBtn.Text             = "SELL"
            sellBtn.TextColor3       = Color3.new(1, 1, 1)
            sellBtn.Font             = Enum.Font.GothamBlack
            sellBtn.TextSize         = 11
            ...
            sellBtn.MouseEnter:Connect(function()
                TweenService:Create(sellBtn, TweenInfo.new(0.1), {BackgroundColor3=Theme.Accent2}):Play()
            end)
            sellBtn.MouseLeave:Connect(function()
                TweenService:Create(sellBtn, TweenInfo.new(0.1), {BackgroundColor3=Theme.Accent1}):Play()
            end)

            local function doSell()
                local ok = pcall(fireproximityprompt, sellPrompt)
                if ok then
                    sellBtn.Text             = "✓"
                    sellBtn.BackgroundColor3 = Color3.fromRGB(30, 180, 80)
            ...
            table.insert(rsAllRows, {btn=sellBtn, prompt=sellPrompt, fn=doSell})
            return row
        end

        -- ── Sell All logic ────────────────────────────────────────────────
        rsSellAll.MouseButton1Click:Connect(function()
            rsSellAll.Text             = "Vendendo..."
            rsSellAll.BackgroundColor3 = Theme.Accent2
            local rows = {}
            for _, e in ipairs(rsAllRows) do
                if e.prompt and e.prompt.Parent and e.prompt.Enabled then
                    table.insert(rows, e)
                end
            end
            task.spawn(function()
                for _, e in ipairs(rows) do
                    pcall(e.fn); task.wait(0.2)
                end
                task.wait(0.3)
                rsSellAll.Text             = "SELL ALL"
                rsSellAll.BackgroundColor3 = Theme.Accent1
        ...
        -- ── Scan ─────────────────────────────────────────────────────────
        local function rsScan()
            for _, ch in ipairs(rsScroll:GetChildren()) do
                if not ch:IsA("UIListLayout") and not ch:IsA("UIPadding") then ch:Destroy() end
            end
            rsAllRows = {}

            local plot = rsFindPlot()
            if not plot then
                rsStatus.Text = "❌ Base não encontrada"
                return
            end

            local podiums = plot:FindFirstChild("AnimalPodiums")
            if not podiums then
                rsStatus.Text = "❌ AnimalPodiums não encontrado"
                return
            end
            ...
            for _, podium in ipairs(podiums:GetChildren()) do
                ...
                if pNum then
                    ...
                    if att then
                        for _, pp in ipairs(att:GetChildren()) do
                            if pp:IsA("ProximityPrompt")
                            and (pp.ActionText or ""):sub(1,4)=="Sell" then
                                local info = rsGetData(plot, pNum)
                                if info then
                                    table.insert(entries, {podiumNum=pNum, prompt=pp, info=info})
                                end
                            end
                        end
                    end
            ...
            for _, e in ipairs(entries) do
                rsAddRow(e.podiumNum, e.prompt, e.info)
            end

            if found == 0 then
                rsStatus.Text              = "Nenhum brainrot para vender"
                rsSellAll.BackgroundColor3 = Theme.SurfaceHighlight
                rsSellAll.TextColor3       = Theme.TextSecondary
            else
                rsStatus.Text              = found .. " brainrot(s) prontos para vender"
                rsSellAll.BackgroundColor3 = Theme.Accent1
                rsSellAll.TextColor3       = Color3.new(1,1,1)
            end
        end
    ...
    -- ── Mini panel toggle button ──────────────────────────────────────────
    local rsBtn = mBtn("Remote Sell: OFF", 61, Theme.Surface, function() end)
    rsBtn.MouseButton1Click:Connect(function()
        if not rsMainFrame then return end
        rsVisible = not rsVisible
        rsMainFrame.Visible = rsVisible
        rsBtn.Text             = rsVisible and "Remote Sell: ON"  or "Remote Sell: OFF"
        rsBtn.BackgroundColor3 = rsVisible and Theme.Accent1      or Theme.Surface
        rsBtn.TextColor3       = rsVisible and Color3.new(0,0,0)  or Theme.TextPrimary
...
local function tpToBestBrainrot()
    ...
    -- Esteira SÓ é usada se nao existe nenhum animal de base disponivel no cache
    -- Antes, quando o cache estava vazio (donos offline descartados), bestBase era nil
    -- e useConv sempre virava true mesmo tendo animals validos na base
                        ...
                        if part and part:IsDescendantOf(Workspace) then
                            ...
                            if not byUser[plr.UserId] then
                                ...
                                tpBtn.MouseButton1Click:Connect(function()
                                    ...
                                    local hrp = char and char:FindFirstChild("HumanoidRootPart")
                                    if not hrp then return end
                                    local row2 = byUser[plr.UserId]
                                    local targetPlot = (row2 and row2.plot) or plot
                                    local targetPos = nil
                                    if targetPlot then
                                        local dh = targetPlot:FindFirstChild("DeliveryHitbox")
                                        if dh and dh:IsA("BasePart") then
                                            targetPos = dh.Position + Vector3.new(0, 6, 0)
                                        end
                                        if not targetPos then
                                            local ap = targetPlot:FindFirstChild("AnimalPodiums")
                                            if ap then targetPos = ap.Position + Vector3.new(0, 6, 0) end
                                        end
                                    end
